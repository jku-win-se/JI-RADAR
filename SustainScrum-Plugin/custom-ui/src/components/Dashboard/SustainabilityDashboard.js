...
function SustainabilityDashboard({ initialProjectKey = null }) {
    ...
    /** Monotonic id so stale async dashboard responses do not overwrite newer sprint/filter state. */
    const dashboardLoadSeq = useRef(0);
    ...
    // Load sprints when project changes (reset sprint selection to avoid stale sprint id from another project)
    useEffect(() => {
    ...
    // Load dashboard when project, sprint, sprint list (for name resolution), or trends window changes
    useEffect(() => {
        if (projectKey && projectKey !== 'PROJ') {
            loadDashboardData();
        } else {
    ...
    }, [projectKey, selectedSprintId, sprints, trendsDays]);
    ...
    const loadSprints = async () => {
        ...
        try {
            ...
            if (result.error) {
                ...
                // Default: "All Sprints" — user selects a sprint explicitly to filter
                setSelectedSprintId('');
            }
        } catch (err) {
    ...
    const loadDashboardData = async () => {
        const reqId = ++dashboardLoadSeq.current;
        setLoading(true);
        setError(null);
        ...
        try {
            ...
            if (reqId !== dashboardLoadSeq.current) {
                return;
            }
            if (data.error) {
                setError(data.error);
    ...
    const handleExport = () => {
        ...
        rows.push(['Trends window', trendsDays === 0 ? 'All time' : `Last ${trendsDays} days`]);
    ...
    return (
        <div className="dashboard-container">
            ...
            <div className="dashboard-controls">
                ...
                <div className="trends-window-wrapper" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <label htmlFor="trends-days" className="sprint-label">Trends:</label>
                    <select
                        id="trends-days"
                        className="sprint-selector"
                        value={trendsDays}
            ...
            {dashboardData && (
                <>
                    {(dashboardData.assessmentCount === 0 || (dashboardData.heatmap || []).length === 0) && (
                        <div className="dashboard-empty-banner" role="status">
                            <strong>No assessments in this view.</strong>{' '}
                            {dashboardData.sprintFilterApplied
                                ? 'This sprint has no assessed issues, or sprint data could not be loaded. Try "All Sprints" or complete assessments for issues in this sprint.'
                                : 'Run assessments from the Sustainability Panel on project issues to see KPIs, heatmap, and trends here.'}
                        </div>
                    )}
                        ...
                        {activeTab === 'trends' && (
                            <div className="trends-content">
                                <TrendsChart 
                                    ...
                                    enabledDimensions={dashboardData.enabledDimensions}
                                    periodLabel={trendsDays === 0 ? 'All time' : `Last ${trendsDays} days`}
                                />
                            </div>
                        )}
                    </div>
