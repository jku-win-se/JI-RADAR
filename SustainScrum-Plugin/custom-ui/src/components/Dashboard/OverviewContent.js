...
function OverviewContent({ dashboardData, kpis, enabledDimensions }) {
    ...
    // Count assessments (prefer server count for sprint-filtered views)
    const assessmentCount = dashboardData.assessmentCount ?? dashboardData.heatmap?.length ?? 0;
    const sprintFiltered = !!dashboardData.sprintFilterApplied;

    return (
        <div className="overview-content">
            {assessmentCount === 0 && (
                <div className="overview-empty-callout" role="status">
                    <p className="overview-empty-title">No assessment data in this view</p>
                    <p className="overview-empty-text">
                        {sprintFiltered
                            ? 'The selected sprint has no issues with a saved sustainability assessment, or sprint membership could not be resolved.'
                            : 'There are no assessed issues in this project yet.'}{' '}
                        Complete assessments in the issue panel, or adjust the sprint filter.
                    </p>
                </div>
            )}
            {/* Overall KPI Summary */}
