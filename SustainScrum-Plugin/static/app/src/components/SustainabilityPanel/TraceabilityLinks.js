    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            if (!(result.issues && result.issues.length) && searchQuery.trim()) {
                setSearchError(`Keine Treffer für „${searchQuery.trim()}“. Prüfe den Key (z. B. SS-7) oder suche per Summary-Text.`);
            }
        } catch (e) {
            setSearchResults([]);
            setSearchError(e.message || 'Suche fehlgeschlagen. Bitte Seite neu laden und erneut versuchen.');
        } finally {
            setSearching(false);
        }
    };
    ...
    const handleShowAll = async () => {
        ...
        try {
            ...
            if (!(result.issues && result.issues.length)) {
                setSearchError('Keine weiteren Issues in diesem Projekt gefunden (außer dem aktuellen).');
            }
        } catch (e) {
            setSearchResults([]);
            setSearchError(e.message || 'Laden fehlgeschlagen. Bitte Seite neu laden und erneut versuchen.');
        } finally {
            setSearching(false);
        }
    };
